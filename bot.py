...
def _sig(uid: str, plan_id: str, exp_iso: str) -> str:
    ...
    [FIX v4.1] توحيد صيغة ISO مع JS .toISOString():
      Python isoformat() → '2025-04-24T14:30:00+00:00'
      JS   .toISOString() → '2025-04-24T14:30:00.000Z'
      الفرق يُنتج Base64 مختلف → توقيع خاطئ في التطبيق!
    """
    # تطبيع: +00:00 → .000Z
    if exp_iso.endswith('+00:00'):
        exp_iso = exp_iso[:-6] + '.000Z'
    elif 'Z' in exp_iso and '.' not in exp_iso.split('T')[-1]:
        exp_iso = exp_iso.replace('Z', '.000Z')
...
def fs_activate(uid: str, order_id: str, app_plan: str) -> bool:
    ...
    try:
        ...
        exp_iso = expires.strftime('%Y-%m-%dT%H:%M:%S.000Z')  # [FIX v4.1] متوافق مع JS .toISOString()
        srv     = _fsv.SERVER_TIMESTAMP

        app_sig  = _sig(uid, app_plan, exp_iso)
        bot_hmac = _hmac_sig(uid, app_plan, exp_iso)
        ...
        def _run(transaction):
            ...
            # [FIX v4.1] منع المعالجة المزدوجة عبر Transaction
            if pay_snap.exists and pay_snap.get('status') == 'completed':
            ...
            # الاحتفاظ بـ requestUsed الحالي عند التجديد [FIX v4.1]
            sub_snap = sub_ref.get(transaction=transaction)
            current_used = 0
            if sub_snap.exists:
                current_used = sub_snap.get('requestUsed') or 0
            ...
            transaction.set(sub_ref, {
                ...
                'requestUsed':      current_used,   # [FIX v4.1] لا يُصفَّر عند التجديد
                'lastAIRequest':    None,
                'expiresAt':        exp_iso,
                'activatedAt':      srv,
                'updatedAt':        srv,
        ...
        transaction = _fdb.transaction()
        _run(transaction)
        logger.info(f"✅ Firestore transaction: uid={uid} plan={app_plan} expires={exp_iso[:10]}")
        return True

    except Exception as e:
        if 'ALREADY_PROCESSED' in str(e):
            logger.warning(f"⚠️ fs_activate: طلب معالج مسبقاً — uid={uid} order={order_id}")
            return True  # ليس خطأ — الاشتراك موجود
...
class DB:
    ...
    def tx_claim(self, tx_id: str, uid: int, method: str, amount: float,
                 plan_name: str, app_plan: str, fb_uid=None,
                 order_id=None, verified_by=None) -> bool:
        """
        [FIX v4.1] atomic check-and-insert لـ tx_id.
        يُدرج السجل فقط إذا لم يكن موجوداً (IGNORE).
        يُعيد True إذا نجح الإدراج → هذا الـ thread يملك التفعيل.
        يُعيد False إذا كان السجل موجوداً → تفعيل مكرر، تجاهل.
        """
        with self._lock:
            c = self.conn.cursor()
            c.execute(
                'INSERT OR IGNORE INTO txs'
                '(tx_id,user_id,method,amount,plan_name,app_plan,fb_uid,order_id,verified_by)'
                ' VALUES(?,?,?,?,?,?,?,?,?)',
                (tx_id, uid, method, amount, plan_name, app_plan,
                 fb_uid, order_id, verified_by))
            self.conn.commit()
            return c.rowcount == 1  # 1 = إدراج جديد، 0 = موجود مسبقاً
...
def _build_pay_info() -> dict:
    """[FIX v4.1] تُحسب في كل استدعاء من متغيرات البيئة مباشرة — لا قيم متجمدة"""
    return {
        'bank':   ('🏦 تحويل بنكي',
                   f'🏛 *بنكك*\n📱 الحساب: `{MY_ACCOUNT}`'),
        'fawry':  ('💳 فوري',
...
# للتوافق مع الاستخدام المباشر في الكود القديم
...
def cb_check(call):
    ...
    if res['ok'] and res['status'] == 'Paid':
        ...
        # [FIX v4.1] atomic claim — يمنع التفعيل المزدوج عند ضغطات متزامنة
        claimed = db.tx_claim(track, uid, 'OxaPay', PLAN_META.get(app_plan, {}).get('usd', 0),
                              plan_name, app_plan, fb_uid=fb_uid,
                              order_id=track, verified_by='oxapay_auto')
        if not claimed:
            bot.edit_message_text(
                '✅ *اشتراكك مفعّل بالفعل!*\n\nهذه الفاتورة معالجة مسبقاً.',
        ...
        try:
            ...
            # أعِد حذف السجل لأن التفعيل فشل
            try:
                db._x('DELETE FROM txs WHERE tx_id=?', (track,))
            except Exception:
                pass
            bot.edit_message_text(
                '❌ *فشل التفعيل*\n\n'
...
def _handle_code(message):
    ...
    try:
        ...
        # [FIX v4.1] تحقق من fb_uid قبل استهلاك الكود — لا نحرق الكود بدون سبب
        if not fb_uid:
            # أعِد حالة الكود لـ NULL — الكود لم يُستخدم فعلياً
            db._x('UPDATE codes SET used_by=NULL, used_at=NULL WHERE code=? AND used_by=?',
                  (code, uid))
            bot.reply_to(message,
                '⚠️ *كود صالح لكن الحساب غير مربوط*\n\n'
                '📋 انسخ معرّفك (UID) من الملف الشخصي في التطبيق وأرسله هنا أولاً،\n'
                'ثم أعِد إدخال الكود مرة أخرى. 🔑',
                reply_markup=_contact_kb(), parse_mode='Markdown')
            return
        ...
        # أعِد حالة الكود لأن التفعيل فشل
        try:
            db._x('UPDATE codes SET used_by=NULL, used_at=NULL WHERE code=? AND used_by=?',
                  (code, uid))
        except Exception:
            pass
        bot.reply_to(message,
            '⚠️ *كود صالح لكن فشل التفعيل*\n\n'
            'تواصل مع المطور وأخبره بالكود لإتمام التفعيل يدوياً.',
            reply_markup=_contact_kb(), parse_mode='Markdown')
...
def handle_photo(message):
    ...
    try:
        ...
        if plan_name:
            ...
            # [FIX v4.1] atomic claim: INSERT OR IGNORE يمنع التفعيل المزدوج
            claimed = db.tx_claim(tx_id, uid, name, amount, plan_name, app_plan,
            ...
            try:
                ...
                # أعِد حذف السجل لأن التفعيل فشل → يسمح بإعادة المحاولة
                try:
                    db._x('DELETE FROM txs WHERE tx_id=?', (tx_id,))
                except Exception:
                    pass
                bot.edit_message_text(
                    '❌ *فشل التفعيل*\n\nتواصل مع المطور',
